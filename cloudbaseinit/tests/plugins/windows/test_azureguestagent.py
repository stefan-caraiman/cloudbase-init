...
class AzureGuestAgentPluginTests(unittest.TestCase):
	...
	def setUp(self, mock_join, mock_exists):
		...
		self.azure_agent = azureguestagent.AzureGuestAgentPlugin()
	...
	@mock.patch('cloudbaseinit.plugins.windows.azureguestagent.os.getenv')
	@mock.patch('cloudbaseinit.plugins.windows.azureguestagent.os.path.join')
	def test_remove_azure_dirs(self, mock_getenv, mock_exists,
								mock_rmtree):
		# mock_join = self.mock_join
		mock_join.side_effect = ["fake folder 1", "fake folder 2"]
	...
	def test_get_os_requirements(self):
		result = self.azure_agent.get_os_requirements()
	...
	@mock.patch('cloudbaseinit.plugins.windows.azureguestagent.factory')
	def _test_execute(self, mock_osutils_factory, mock_get_sourcepath,
					  mock_remove_agent, mock_remove_dirs, mock_join, 
					  mock_exists, provisioning_data):
		...
		if provisioning_data is None:
			...
			if not ga_package_name:
				...
				with self.assertRaises(ex) as exc:
					self.azure_agent.execute(mock_service, mock.sentinel)
			expected_logging = [
