...
MOCK_RAW_LINK = {
    constant.ID: mock.sentinel.link_id,
    constant.NAME: mock.sentinel.name,
    constant.TYPE: mock.sentinel.type,
    constant.NEUTRON_PORT_ID: mock.sentinel.port_id,
    constant.MAC_ADDRESS: fake_json_response.MAC0,
    constant.MTU: mock.sentinel.mtu,
    constant.BOND_LINKS: mock.sentinel.bondlinks,
    constant.BOND_MODE: mock.sentinel.bondmode,
    constant.BOND_MIIMON: mock.sentinel.miimon,
    constant.BOND_HASH_POLICY: mock.sentinel.hash,
    constant.VIF_ID: mock.sentinel.vif_id,
    constant.VLAN_ID: mock.sentinel.vlan_id,
    constant.VLAN_LINK: mock.sentinel.vlan_link,
    constant.PRIORITY: mock.sentinel.priority}
MOCK_RAW_NETWORK = {
    constant.ID: mock.sentinel.network_id,
    constant.IP_ADDRESS: fake_json_response.ADDRESS0,
    constant.VERSION: mock.sentinel.version,
    constant.NETMASK: fake_json_response.NETMASK0,
    constant.GATEWAY: mock.sentinel.gateway,
    constant.DNS: mock.sentinel.dns_namservers,
    constant.BROADCAST: mock.sentinel.broadcast,
    constant.ASSIGNED_TO: mock.sentinel.assigned_to,
    constant.NEUTRON_NETWORK_ID: mock.sentinel.network_id,
    constant.PRIORITY: mock.sentinel.priority}
MOCK_RAW_ROUTE = {
    constant.ID: mock.sentinel.route_id,
...
class TestNetworkDetailsBuilder(unittest.TestCase):
    ...
    def test_get_field_missing(self):
        field_name = constant.IP_ADDRESS
    ...
    def test_get_field_not_required(self):
        field = base_ns.NetworkDetailsBuilder._Field(name=constant.IP_ADDRESS)
        res = self._network_builder._get_field(field, {})
        self.assertEqual(res, (constant.IP_ADDRESS, None))
        # Cover for a callable default
        mock_callable_default = mock.MagicMock(return_value=None)
        field = (self._network_builder._Field(name=constant.IP_ADDRESS,
                                              default=mock_callable_default))
        res = self._network_builder._get_field(field, {})
        self.assertEqual(res, (constant.IP_ADDRESS, None))

    def test_get_field(self):
        mock_field = self._network_builder._Field(name=constant.IP_ADDRESS,
                                                  alias=["ip", "IP"])
        mock_raw_data = {constant.IP_ADDRESS: mock.sentinel.ip}
        res = self._network_builder._get_field(mock_field, mock_raw_data)
        expected_fields = (constant.IP_ADDRESS, mock.sentinel.ip)
    ...
    def test_process_links_invalid_links(self):
        self._network_builder._links = {mock.sentinel.link: {
            constant.IP_ADDRESS: mock.sentinel.values,
            constant.MAC_ADDRESS: fake_json_response.MAC0
        }
        }
    ...
    def test_process_networks_invalid(self, mock_process_interface):
        self._network_builder._networks = {mock.sentinel.link: {
            constant.IP_ADDRESS: mock.sentinel.ip,
            constant.NETMASK: mock.sentinel.netmask
        }
        }
        self.assertRaises(exception.NetworkDetailsError,
