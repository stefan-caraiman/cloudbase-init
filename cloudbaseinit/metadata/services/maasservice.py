...
class MaaSHttpService(base.BaseHTTPMetadataService):
    ...
    def _http_request(self, url, data=None, headers=None):
        ...
        LOG.debug("Got url: {}".format(url))
        LOG.debug("Got data: {}".format(data))
        LOG.debug("Got headers: {}".format(headers))
        if not url.startswith("http"):
            url = requests.compat.urljoin(self._base_url, url)
        if headers is None:
            headers = self._get_oauth_headers(url)
        LOG.debug("Changed url: {}".format(url))
        LOG.debug("Changed data: {}".format(data))
        LOG.debug("Changed headers: {}".format(headers))
