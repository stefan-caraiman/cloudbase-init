...
class WindowsUtils(base.BaseOSUtils):
    ...
    def add_user_to_local_group(username, groupname):
        ...
        group_name = encoding.get_as_string(groupname)
        try:
            win32net.NetLocalGroupAddMembers(None, group_name,
                                             3, [user_group_info])
        except win32net.error as ex:
            raise exception.WindowsCloudbaseInitException(
                "Failed adding %(user)s to %(group)s: %(reason)s" %
                {"user": username, "group": group_name, "reason": ex})
